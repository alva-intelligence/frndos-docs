...
| Section | Type | What to do |
|---|---|---|
| The At-a-Glance Overview | 📸 screenshot | add screenshot of the Customer Intelligence overview (four cards + tabs) → /img/guides-asset/<name>.jpeg |
| Before You Start | ✍️ confirm from live UI | confirm how a user navigates to Customer Intelligence (which sidebar/menu entry opens it) — the entry point isn't readable from the page code. Add the exact path + screenshot. |
| Commenting on What You're Looking At | 📸 screenshot | add screenshot of Customer Intelligence in comment mode — pin on a customer row + the Comments dock → /img/guides-asset/<name>.jpeg |

### `docs/research/importing-customers.mdx` — Importing & Enriching Customers

| Section | Type | What to do |
|---|---|---|
| Step 1 — Open the Integrations Tab | 📸 screenshot | add screenshot of the Integrations tab upload area → /img/guides-asset/<name>.jpeg |
| Step 3 — Choose How Many to Enrich | ✍️ confirm from live UI | confirm the sampling modal's exact fields (custom count vs. percentage, credit cost display) with a screenshot. |

### `docs/research/exploring-customers-and-segments.mdx` — Exploring Customers & Segments

| Section | Type | What to do |
|---|---|---|
| Customer Table View | 📸 screenshot | add screenshot of the Customer Table View → /img/guides-asset/<name>.jpeg |
| Opening a Customer Profile | ✍️ confirm from live UI | confirm the exact profile-drawer layout and section names with a screenshot. |
| Segment List | 📸 screenshot | add screenshot of the Segment List → /img/guides-asset/<name>.jpeg |
| Segment List | ✍️ confirm from live UI | confirm how a user CREATES a manual segment (the exact action/flow) — not fully readable from the list view code. |

### `docs/research/customer-insights.mdx` — Getting Customer Insights

| Section | Type | What to do |
|---|---|---|
| Step 1 — Describe Your Audience | 📸 screenshot | add screenshot of the Customer Insights filter prompt → /img/guides-asset/<name>.jpeg |
| Step 2 — Show the Results | ✍️ confirm from live UI | confirm the full set of result sections and their exact labels with a screenshot (some section titles are data-driven). |

### `docs/research/audience-personas.mdx` — Audience Personas — From Followers to People

| Section | Type | What to do |
|---|---|---|
| Opening It | 📸 screenshot | add screenshot of the Audience Personas page — three tabs, Segments/Personas metric cards, persona grid → /img/guides-asset/<name>.jpeg |
| Segments | ✍️ confirm from live UI | confirm from live UI — segment naming and duplicate merging happen server-side during generation and aren't readable from the web code, so the before/after wording here can't be quoted from anything. Confirm the current naming style with a screenshot of a real Segments tab. |
| Commenting on Personas and Segments | 📸 screenshot | add screenshot of Audience Personas in comment mode — pin on a persona card + the Comments dock → /img/guides-asset/<name>.jpeg |
