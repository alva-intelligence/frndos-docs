...
| Connecting Another AI App | 🔒 third-party UI | confirm from live UI — the per-app menu path for ChatGPT, Cursor, Claude Desktop, Claude Code and Gemini, then add a short per-app list under this section. Those screens belong to the AI app vendors and change independently. This section is the intended destination of a "Setup for your app →" link from McpConnectModal.tsx (frnd-web), so it must eventually answer the non-Claude cases. |
